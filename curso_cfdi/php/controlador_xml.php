...
$informacion = ControladorAgregarDatos::EndRegister();
$empid = $informacion[0][0];
$cfdiserid = $informacion[0][1];
$cfdiid = $informacion[0][2];
//echo "<br>info:".$informacion;

//$resultado_genera =ControladorAgregarDatos::getCountCFDI();
//$cfdiid = $resultado_genera - 1;
//echo "sigue".$cfdiid;
...
if( $modo == 4 ){
	...
	//echo "antes<br>";
	//var_dump($m_info);
	//echo "<br>fiiiiiiiiiiiinnnnnnnnnnn<br>";
	//-MANE-numero de id
	$cfdi_xml1->empid = $empid;
	$cfdi_xml1->cfdiserid = $cfdiserid;
	$cfdi_xml1->cfdiid = $cfdiid;
	...
	*/
	//$xml_carpeta = "../../../timbradoMANE/".$empid."/xml/";
	//$xml_carpeta_cfdi = "../../../timbradoMANE/".$empid."/cfdi/";
	...
	//$xml_carpeta_pdf = "../../../timbradoMANE/".$empid."/pdf/";

	//unlink($xml_carpeta);
	//unlink($xml_carpeta_cfdi);
	//unlink($xml_carpeta_pdf);
	// Crear carpetas:
	...
	if( file_exists( $xml_archivo_cfdi)){
		...
		$archivo_xml = trim(file_get_contents( $xml_archivo_cfdi ));
		 //echo "<br>A)<textarea cols='80' rows='10'> $archivo_xml </textarea>"; // exit;
		// Convertir a Matriz:
		$p = xml_parser_create();
		...
		die('<br> El CFDI ya existe: '. $xml_archivo_cfdi );
	}
	//echo "aqui...:<br>";
	...
	// Leer el XML y actualizar la tabla cfdi:
	//$archivo_xml = trim(file_get_contents( $xml_archivo_cfdi ));
	$archivo_xml = trim( $xml_archivo_cfdi );
