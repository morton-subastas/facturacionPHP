...
if (($fac != '')) {
  ...
  echo "<th  bgcolor='#004D43' style='color:#ffffff' width='5%'>LOTE</th>";
  ...
  while ($suma < $cuantos) {
    ...
    if ($conn->connect_error) {
      ...
      if ($trae2 == 0) {
        ...
        echo "</label><br>";
        echo "<label class='miro-radiobutton'>";
        echo "<input type='radio' name='radio" . $suma . "' id='radio" . $suma . "' value='Andromaco' onchange='cambia_direccionAndromaco($suma)'>";
        echo "<span>Andrómaco</span>";
        echo "</label><br>";
        echo "<label class='miro-radiobutton'>";
        ...
        if (($variable[$suma]['email'] == $arreglo_cuantos[$arranca][0])  && ($suma == $arreglo_cuantos[$arranca][2])) {
          ...
          $bidder = $variable[$suma]["bidder"];
            ...
            echo "<input type='hidden' id='paleta' name='paleta' value='". substr($variable[$suma]["salelot"], 5, 12) ."'>";
...
<script>
  ...
  function cambia_direccionA179(suma) {
    ...
    $.ajax({
      data: parametros, //datos que se envian a traves de ajax
      url: 'ajax_proceso4A179', //archivo que recibe la peticion
      type: 'POST', //método de envio
      beforeSend: function() {
        $("#resultado4").html("<p style='color:green';>Agregado, con éxito...</p>");
      },
  ...
  function cambia_direccionC(suma) {
    ...
    $.ajax({
      data: parametros, //datos que se envian a traves de ajax
      url: 'ajax_proceso4C', //archivo que recibe la peticion
      type: 'post', //método de envio
      beforeSend: function() {
        $("#resultado4").html("<p style='color:green';>Agregado, con éxito...</p>");
      },
  ...
  function cambia_direccionAndromaco(suma) {
    var s = document.getElementById("subasta").value;
    ...
    $.ajax({
      data: parametros, //datos que se envian a traves de ajax
      url: 'ajax_procesoAndromaco', //archivo que recibe la peticion
      type: 'post', //método de envio
      beforeSend: function() {
        $("#resultado4").html("<p style='color:green';>Agregado, con éxito...</p>");
      },
